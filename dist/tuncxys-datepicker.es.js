...
class b {
  constructor(e, t = {}) {
    ...
    this.onDocumentClick = (d) => {
      const m = this.dom && this.dom.wrapper && this.dom.wrapper.contains(d.target), h = this.dom && this.dom.calendarPopup && this.dom.calendarPopup.contains(d.target), c = this.dom && this.dom.timePopup && this.dom.timePopup.contains(d.target);
      !m && !h && !c && this.closePickers();
    }, this.onWindowResize = () => {
    ...
    const s = (d) => {
      if (!d) return null;
      if (d instanceof Date) return isNaN(d.getTime()) ? null : new Date(d.setHours(0, 0, 0, 0));
      if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return null;
      const h = d.split("-"), c = parseInt(h[0], 10), u = parseInt(h[1], 10), p = parseInt(h[2], 10);
      if (u < 1 || u > 12) return null;
      const g = new Date(c, u - 1, p);
      return g.getFullYear() !== c || g.getMonth() !== u - 1 || g.getDate() !== p ? null : (g.setHours(0, 0, 0, 0), g);
    }, r = (d, m) => Math.round((d - m) / (1e3 * 60 * 60 * 24));
    let a = s(t.dateUpperLimit), o = s(t.dateLowerLimit);
    (t.dateUpperLimit && !a || t.dateLowerLimit && !o) && (console.warn(this.lang.console.limitWarning), this.enableLimit = !1, this.enableDayLimit = !1, this.minOffset = 100, this.maxOffset = 100, a = null, o = null);
    const l = new Date(this.realToday.getFullYear(), this.realToday.getMonth(), this.realToday.getDate());
    if (this.enableDayLimit === !0 && (a || o))
      if (a && o && a < o && ([a, o] = [o, a]), a && !o) {
        const d = r(a, l);
        this.maxOffset = d, this.minOffset -= d;
      } else if (!a && o) {
        const d = r(l, o);
        this.minOffset = d, this.maxOffset += r(o, l);
      } else
        this.maxOffset = r(a, l), this.minOffset = r(l, o);
    ...
    ), n = 11), this.enableTime && this.SEGMENTS.push(
      { type: "hour", start: n, end: n + 2, maxLen: 2, min: 0, max: 23 },
      { type: "minute", start: n + 3, end: n + 5, maxLen: 2, min: 0, max: 59 },
      { type: "second", start: n + 6, end: n + 8, maxLen: 2, min: 0, max: 59 }
  ...
  setPopupPosition(e) {
    ...
    const r = e.offsetWidth, a = e.offsetHeight || 350;
    let o = t.bottom + i + 8;
    window.innerHeight - t.bottom < a && t.top > a && (o = t.top + i - a - 8);
    let n = t.left + s + t.width / 2 - r / 2;
  ...
  applyTheme() {
    ...
    if (this.theme === "custom" && this.colors) {
      ...
      for (const [r, a] of Object.entries(i))
        this.colors[r] && s.push(`${a}: ${this.colors[r]}`);
      e = s.join(";");
  ...
  bindEvents() {
    const { input: e, calendarBtn: t, clockBtn: i, prevBtn: s, nextBtn: r, monthYearDisplay: a, controlBtns: o } = this.dom;
    document.addEventListener("click", this.onDocumentClick), window.addEventListener("resize", this.onWindowResize), window.addEventListener("scroll", this.onWindowResize), o && o.length > 0 && o.forEach((n) => {
      n.addEventListener("click", (d) => {
        d.preventDefault(), d.stopPropagation(), d.stopImmediatePropagation();
        const m = n.dataset.unit, h = n.dataset.dir;
        this.scrollTimeButton(m, h);
      ...
      for (let d = 0; d < this.SEGMENTS.length; d++)
        if (n >= this.SEGMENTS[d].start && n <= this.SEGMENTS[d].end) {
          this.inputBuffer.length > 0 && this.activeSegmentIndex !== d && this.finalizeInput(this.activeSegmentIndex), this.activeSegmentIndex = d, this.selectSegment(d);
          break;
        }
    ...
    t && (t.addEventListener("click", (n) => {
      ...
      n.stopPropagation(), !r.classList.contains("disabled") && (this.currentMonth++, this.currentMonth > 11 && (this.currentMonth = 0, this.currentYear++), this.renderCalendar(this.currentMonth, this.currentYear));
      ...
      const d = n.target.closest(".txydp-day");
      if (!d || d.style.visibility === "hidden" || d.style.pointerEvents === "none") return;
      n.stopPropagation();
      const m = parseInt(d.innerText);
      d.classList.contains("other-month") && (m > 15 ? (this.currentMonth--, this.currentMonth < 0 && (this.currentMonth = 11, this.currentYear--)) : (this.currentMonth++, this.currentMonth > 11 && (this.currentMonth = 0, this.currentYear++))), this.selectedDay = m, this.selectedMonth = this.currentMonth, this.selectedYear = this.currentYear, this.renderCalendar(this.currentMonth, this.currentYear), this.updateFullInputString();
    }));
  }
  destroy() {
  ...
  updateHiddenInput() {
    ...
    let e = [];
    if (this.enableDate) {
      let i = this.selectedYear.toString().padStart(4, "0"), s = (this.selectedMonth + 1).toString().padStart(2, "0"), r = this.selectedDay.toString().padStart(2, "0");
      e.push(`${i}-${s}-${r}`);
    }
  ...
  scrollTimeButton(e, t) {
    ...
    const s = 32, r = i.scrollTop;
    t === "down" ? i.scrollTo({ top: r + s, behavior: "smooth" }) : i.scrollTo({ top: r - s, behavior: "smooth" });
  }
  getParsedDateFromInput() {
    ...
    let r = parseInt(e.substring(t.start, t.end)), a = parseInt(e.substring(i.start, i.end)) - 1, o = parseInt(e.substring(s.start, s.end));
    return { d: r, m: a, y: o };
  }
  handleKeydown(e) {
  ...
  finalizeInput(e, t = !1) {
    ...
    let r = s;
    if (i.type === "year" && this.inputBuffer.length > 0 && this.inputBuffer.length !== 4 && this.inputBuffer.length !== 2 && (s = i.min), s < i.min && (s = i.min), i.max && s > i.max && (s = i.max), s !== r) {
      this.triggerErrorEffect();
      let o = i.type === "year" ? this.lang.warnings.invalidYear.replace("{year}", s) : this.lang.warnings.invalidDate.replace("{date}", s);
      this.showWarningMessage(o);
    }
  ...
  validateDayCountInMonth() {
    ...
    if (e.d > t) {
      ...
      let s = i.getDate().toString().padStart(2, "0"), r = (i.getMonth() + 1).toString().padStart(2, "0"), a = i.getFullYear().toString().padStart(4, "0"), o = `${s}/${r}/${a}`, l = this.lang.warnings.invalidDate.replace("{date}", `<strong>${o}</strong>`);
      this.showWarningMessage(l);
      let n = this.dom.input.value, m = `${s}/${r}/${a}`;
      this.enableTime && n.length > 10 && (m += n.substring(10)), this.dom.input.value = m, this.selectedDay = i.getDate(), this.selectedMonth = i.getMonth(), this.selectedYear = i.getFullYear(), this.renderCalendar(this.selectedMonth, this.selectedYear), this.updateHiddenInput();
    }
  }
  updateInputValue(e, t, i = !1, s = !0, r = !0) {
    let a = this.dom.input.value;
    const o = this.SEGMENTS.find((n) => n.type === e);
    if (!o) return;
    let l = a.substring(0, o.start) + t + a.substring(o.end);
    if (this.dom.input.value = l, !(!i && !s)) {
      if (this.enableTime) {
        const n = this.SEGMENTS.find((p) => p.type === "hour"), d = this.SEGMENTS.find((p) => p.type === "minute"), m = this.SEGMENTS.find((p) => p.type === "second");
        let h = l.substring(n.start, n.end), c = l.substring(d.start, d.end), u = l.substring(m.start, m.end);
        isNaN(parseInt(h)) || (this.selectedHour = h), isNaN(parseInt(c)) || (this.selectedMinute = c), isNaN(parseInt(u)) || (this.selectedSecond = u), ["hour", "minute", "second"].includes(e) && this.scrollToValues(this.selectedHour, this.selectedMinute, this.selectedSecond);
      }
      if (this.enableDate) {
        let n = parseInt(l.substring(this.SEGMENTS.find((h) => h.type === "day").start, this.SEGMENTS.find((h) => h.type === "day").end)), d = parseInt(l.substring(this.SEGMENTS.find((h) => h.type === "month").start, this.SEGMENTS.find((h) => h.type === "month").end)) - 1, m = parseInt(l.substring(this.SEGMENTS.find((h) => h.type === "year").start, this.SEGMENTS.find((h) => h.type === "year").end));
        if (!isNaN(n) && !isNaN(d) && !isNaN(m) && (this.selectedDay = n, this.selectedMonth = d, this.selectedYear = m, ["day", "month", "year"].includes(e))) {
          let h = new Date(this.selectedYear, this.selectedMonth, this.selectedDay), c = new Date(this.minDate);
          c.setHours(0, 0, 0, 0);
          let u = new Date(this.maxDate);
          u.setHours(0, 0, 0, 0);
          let p = !0;
          if (h < c && (s ? (this.selectedYear = this.minDate.getFullYear(), this.selectedMonth = this.minDate.getMonth(), this.selectedDay = this.minDate.getDate()) : p = !1), h > u && (s ? (this.selectedYear = this.maxDate.getFullYear(), this.selectedMonth = this.maxDate.getMonth(), this.selectedDay = this.maxDate.getDate()) : p = !1), s && (h < c || h > u)) {
            this.triggerErrorEffect();
            let g = h < c ? c : u, y = `${g.getDate().toString().padStart(2, "0")}/${(g.getMonth() + 1).toString().padStart(2, "0")}/${g.getFullYear()}`, S = this.lang.warnings.invalidDate.replace("{date}", y);
          ...
          if ((p || s) && (this.currentYear = this.selectedYear, this.currentMonth = this.selectedMonth, this.renderCalendar(this.currentMonth, this.currentYear), s && r)) {
  ...
  populateTimeColumn(e, t, i) {
    ...
    for (let a = 0; a < t; a++) {
      const o = a.toString().padStart(2, "0"), l = document.createElement("div");
      l.className = "txydp-time-cell", l.innerText = o, l.dataset.val = o, e.appendChild(l);
    }
    const r = document.createElement("div");
    r.className = "txydp-spacer", e.appendChild(r), e.addEventListener("scroll", () => {
      this.handleScroll(e, i);
    }), e.addEventListener("wheel", (a) => {
      a.preventDefault();
      const o = 32, l = a.deltaY > 0 ? 1 : -1, n = e.scrollTop + l * o;
      e.scrollTo({ top: n, behavior: "smooth" });
    }, { passive: !1 });
  }
  handleScroll(e, t) {
    const s = e.scrollTop, r = Math.round(s / 32), a = e.querySelectorAll(".txydp-time-cell");
    if (a.forEach((o) => o.classList.remove("active")), a[r]) {
      a[r].classList.add("active");
      const o = a[r].dataset.val;
      t === "hour" && (this.selectedHour = o), t === "minute" && (this.selectedMinute = o), t === "second" && (this.selectedSecond = o), this.updateFullInputString();
    }
  }
  scrollToValues(e, t, i) {
    const r = (a, o) => {
      const l = parseInt(o);
      a.scrollTop = l * 32;
      const n = a.querySelectorAll(".txydp-time-cell");
      n.forEach((d) => d.classList.remove("active")), n[l] && n[l].classList.add("active");
    };
    r(this.dom.hourCol, e), r(this.dom.minuteCol, t), r(this.dom.secondCol, i);
  }
  updateFullInputString() {
    let e = [];
  ...
  showWarningMessage(e) {
    ...
    t.innerHTML = e;
    const i = this.dom.inputGroup.getBoundingClientRect(), s = window.scrollY || document.documentElement.scrollTop, r = window.scrollX || document.documentElement.scrollLeft, a = i.left + r + i.width / 2, o = i.top + s - 10;
    t.style.left = `${a}px`, t.style.top = `${o}px`, t.classList.add("visible"), this.warningTimeout && clearTimeout(this.warningTimeout), this.warningTimeout = setTimeout(() => {
  ...
  renderCalendar(e, t) {
    ...
    s.setHours(0, 0, 0, 0);
    let r = new Date(t, e - 1, 1), a = new Date(t, e + 1, 1);
    r < i && r.getMonth() !== i.getMonth() ? this.dom.prevBtn.classList.add("disabled") : this.dom.prevBtn.classList.remove("disabled"), a > s && a.getMonth() !== s.getMonth() ? this.dom.nextBtn.classList.add("disabled") : this.dom.nextBtn.classList.remove("disabled");
    let o = new Date(t, e, 1).getDay(), l = o === 0 ? 6 : o - 1, n = new Date(t, e + 1, 0).getDate(), d = new Date(t, e, 0).getDate();
    for (let h = 0; h < l; h++) {
      const c = d - l + 1 + h, u = document.createElement("div");
      u.classList.add("txydp-day", "other-month"), u.innerText = c;
      let p = new Date(t, e - 1, c);
      p < i && (u.style.visibility = "hidden", u.style.pointerEvents = "none"), this.isDateRestricted(p) && (u.style.pointerEvents = "none"), this.dom.calendarDays.appendChild(u);
    }
    for (let h = 1; h <= n; h++) {
      const c = document.createElement("div");
      c.classList.add("txydp-day"), c.innerText = h;
      let u = new Date(t, e, h);
      u < i || u > s ? (c.style.visibility = "hidden", c.style.pointerEvents = "none") : this.isDateRestricted(u) && c.classList.add("restricted-day"), this.selectedDay !== null && h === this.selectedDay && e === this.selectedMonth && t === this.selectedYear && !c.classList.contains("restricted-day") && c.classList.add("selected"), h === this.realToday.getDate() && e === this.realToday.getMonth() && t === this.realToday.getFullYear() && c.classList.add("today"), this.dom.calendarDays.appendChild(c);
    }
    const m = l + n;
    for (let h = 1; h <= 42 - m; h++) {
      const c = document.createElement("div");
      c.classList.add("txydp-day", "other-month"), c.innerText = h;
      let u = new Date(t, e + 1, h);
      u > s && (c.style.visibility = "hidden", c.style.pointerEvents = "none"), this.isDateRestricted(u) && (c.style.pointerEvents = "none"), this.dom.calendarDays.appendChild(c);
    }
  }
}
